...
export const MODULE_DOCS: Record<string, string> = {
  ADEmbedding: `
    ...
    <p>During backprop, only the rows that were looked up receive gradient updates. This makes embedding layers sparse in practice&mdash;most of the weight matrix doesn't get touched on any given step.</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>vocab_size</strong> &mdash; number of distinct tokens the layer can handle</li>
      <li><strong>embed_dim</strong> &mdash; length of each token's vector</li>
    </ul>
    <h3>Outputs</h3>
    <ul>
      <li><strong>output</strong> &mdash; the embedded sequence <code>[embed_dim x seq_len]</code></li>
      <li><strong>weights</strong> (optional) &mdash; the raw weight matrix, useful for weight tying with the output projection</li>
    </ul>
  ...
  ADMultiHeadAttention: `
    ...
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong> &mdash; must be divisible by num_heads</li>
      <li><strong>num_heads</strong> &mdash; more heads = finer-grained attention patterns, but each head works in a smaller subspace</li>
    </ul>
  `,

  ADFeedForward: `
    <h2>Feed-Forward Network (GELU)</h2>
    <p>Two linear layers with a GELU activation between them: <code>FFN(x) = W2 * GELU(W1 * x + b1) + b2</code>. Applied independently to each token position&mdash;there's no interaction between positions here, that's attention's job.</p>
    ...
    <p>GELU (Gaussian Error Linear Unit) is a smooth activation that looks like ReLU but doesn't have a hard zero cutoff&mdash;it allows small negative gradients through, which tends to help with training stability.</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong> &mdash; input and output size</li>
      <li><strong>hidden_dim</strong> &mdash; intermediate size (try 4x embed_dim as a starting point)</li>
    </ul>
  ...
  ADLayerNorm: `
    ...
    <p>The normalization is computed per-column (per token position), across the feature dimension. This keeps activation magnitudes stable as they pass through many layers&mdash;without it, values tend to either explode or vanish after a few transformer blocks.</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>dim</strong> &mdash; feature dimension (should match embed_dim)</li>
      <li><strong>eps</strong> &mdash; added inside the sqrt to prevent division by zero. Default 1e-5. You probably don't need to change this.</li>
    </ul>
  ...
  ADMoE: `
    ...
    <p>Replaces the single FFN in a transformer block with <code>num_experts</code> independent FFN networks and a learned router. For each token, only the top-k scoring experts actually run&mdash;the rest are skipped entirely.</p>
    ...
    <p>Without regularization, the router tends to collapse&mdash;it learns to send everything to one or two experts and ignores the rest. The auxiliary loss penalizes uneven expert utilization by comparing the fraction of tokens routed to each expert against a uniform target. This loss gets added to the main training loss (usually with a small coefficient like 0.01).</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong> &mdash; input/output dimension</li>
      <li><strong>hidden_dim</strong> &mdash; hidden size inside each expert FFN</li>
      <li><strong>num_experts</strong> &mdash; total expert count (8 and 16 are common choices)</li>
      <li><strong>top_k</strong> &mdash; how many experts each token activates (usually 2)</li>
    </ul>
...
x = x + FFN(Norm(x))</code></pre>
    <p>The normalization (LayerNorm or RMSNorm) goes <em>before</em> the sublayer, not after. This is the "pre-norm" architecture&mdash;it's more stable to train than post-norm, especially at depth.</p>
    <p>The FFN slot can be filled by one of three variants depending on the config: standard GELU FFN, SwiGLU, or MoE. The block handles this internally&mdash;from the outside, it's just input tensor in, output tensor out.</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong>, <strong>hidden_dim</strong>, <strong>n_heads</strong> &mdash; the core dimensions</li>
      <li><strong>use_moe</strong> &mdash; swap the FFN for a MoE layer</li>
      <li><strong>num_experts</strong>, <strong>moe_top_k</strong> &mdash; MoE settings (ignored if use_moe is false)</li>
    </ul>
  ...
  ADLinear: `
    ...
    <h3>Parameters</h3>
    <ul>
      <li><strong>input_dim</strong> &mdash; number of input features (rows of x)</li>
      <li><strong>output_dim</strong> &mdash; number of output features (rows of y)</li>
    </ul>
  `,
  ...
  ADPositionalEncoding: `
    ...
    <p>This is the simplest form of position encoding&mdash;it works, but it can't generalize to sequence lengths longer than <code>max_len</code>. For length-agnostic alternatives, see RoPE (which encodes position through rotation) or ALiBi (which adds position bias directly to attention scores).</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong> &mdash; must match the token embedding dimension</li>
      <li><strong>max_len</strong> &mdash; the longest sequence you'll ever feed in (default 512)</li>
    </ul>
  `,
  ...
  Tokenizer: `
    ...
    <h3>Parameters</h3>
    <ul>
      <li><strong>vocab_file</strong> &mdash; maps characters/tokens to integer IDs</li>
      <li><strong>bpe_codes</strong> &mdash; merge rules file (optional, leave empty for character-level)</li>
    </ul>
  `,
  ...
  TextInput: `
    ...
    <h3>Config</h3>
    <ul>
      <li><strong>text</strong> &mdash; the input string</li>
    </ul>
  `,
  ...
  TokenIDsInput: `
    ...
    <h3>Config</h3>
    <ul>
      <li><strong>tokens</strong> &mdash; array of integer token IDs, e.g. <code>[1, 2, 3, 4]</code></li>
    </ul>
  `,
  ...
  IntInput: `
    ...
    <h3>Config</h3>
    <ul>
      <li><strong>value</strong> &mdash; the integer</li>
    </ul>
  `,

  SeqLenExtractor: `
    <h2>Sequence Length Extractor</h2>
    <p>Takes a token ID list and outputs its length as an integer. Mostly a convenience node&mdash;connect it between a tokenizer output and a positional encoding's <code>seq_len</code> input.</p>
  `,
  ...
  CrossEntropy: `
    ...
    <p>Uses log-sum-exp for numerical stability&mdash;without it, the exp() in softmax overflows on large logit values. The implementation subtracts the per-column max before exponentiating.</p>
    <h3>Inputs</h3>
    <ul>
      <li><strong>logits</strong> &mdash; raw model output, shape <code>[vocab_size x seq_len]</code></li>
      <li><strong>targets</strong> &mdash; ground truth token IDs, one per position</li>
    </ul>
    <h3>Output</h3>
  ...
  ADRMSNorm: `
    ...
    <p>There's no mean centering and no beta (shift) parameter&mdash;just a learnable scale <code>gamma</code> (initialized to 1). This makes it cheaper to compute and has one fewer parameter per dimension. In practice, the missing mean centering doesn't seem to hurt quality.</p>
    ...
    <h3>Parameters</h3>
    <ul>
      <li><strong>dim</strong> &mdash; feature dimension</li>
      <li><strong>eps</strong> &mdash; stability constant, default 1e-6 (note: smaller default than LayerNorm's 1e-5)</li>
    </ul>
    <h3>When to use</h3>
  ...
  ADSwiGLU: `
    ...
    <p>The gating means the network can learn to selectively zero out dimensions&mdash;if the gate output is near zero for some feature, that feature gets suppressed regardless of what the up-projection computes. This added control tends to produce better loss curves than a plain GELU FFN.</p>
    ...
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong> &mdash; input and output dimension</li>
      <li><strong>hidden_dim</strong> &mdash; intermediate dimension for all three projections</li>
    </ul>
  `,
  ...
  RoPE: `
    ...
    <h3>Parameters</h3>
    <ul>
      <li><strong>head_dim</strong> &mdash; dimension of each attention head (must be even, since we rotate pairs)</li>
      <li><strong>max_len</strong> &mdash; precompute cos/sin tables up to this length (default 4096, but can extrapolate beyond)</li>
    </ul>
  ...
  ADGQA: `
    ...
    <ul>
      <li>W_q: <code>[64 x 64]</code> &mdash; full size, produces all 8 Q heads</li>
      <li>W_k: <code>[16 x 64]</code> &mdash; smaller, produces only 2 KV heads</li>
      <li>W_v: <code>[16 x 64]</code> &mdash; same as K</li>
      <li>W_o: <code>[64 x 64]</code> &mdash; full size output projection</li>
    </ul>
    ...
    <h3>Why bother</h3>
    <p>The KV cache during inference is proportional to the number of KV heads. Going from 8 KV heads to 2 cuts KV cache memory by 4x. The quality tradeoff is small&mdash;GQA with 2 KV heads is much closer in quality to full MHA than you'd expect.</p>
    <h3>Special cases</h3>
    <ul>
      <li><code>num_kv_heads = 1</code> &mdash; Multi-Query Attention (MQA), all heads share one KV</li>
      <li><code>num_kv_heads = num_heads</code> &mdash; standard Multi-Head Attention</li>
    </ul>
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong> &mdash; total model dimension</li>
      <li><strong>num_heads</strong> &mdash; number of query heads</li>
      <li><strong>num_kv_heads</strong> &mdash; number of key/value heads (must divide num_heads evenly)</li>
    </ul>
  `,
  ...
  ADLoRA: `
    ...
    <ul>
      <li><code>W</code> &mdash; <code>[out x in]</code>, frozen (not in the optimizer's parameter list)</li>
      <li><code>A</code> &mdash; <code>[rank x in]</code>, trainable, initialized with Xavier uniform</li>
      <li><code>B</code> &mdash; <code>[out x rank]</code>, trainable, initialized to <strong>zero</strong></li>
    </ul>
    <p>Because B starts at zero, the initial output is exactly <code>W @ x + bias</code>&mdash;the LoRA contribution is zero at the start. As training progresses, B and A learn a low-rank correction to the frozen weights.</p>
    ...
    <p>The LoRA output is scaled by <code>alpha/rank</code>. When <code>alpha = rank</code> (the default), the scale is 1. Setting alpha higher makes the LoRA contribution larger relative to the base weights. This is a hyperparameter you can tune&mdash;higher alpha means the adaptation has more influence.</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>input_dim</strong>, <strong>output_dim</strong> &mdash; dimensions of the linear layer being adapted</li>
      <li><strong>rank</strong> &mdash; rank of the low-rank matrices (default 8)</li>
      <li><strong>alpha</strong> &mdash; scaling factor (default 8.0, same as rank)</li>
    </ul>
  `,
  ...
  ADFlashAttention: `
    ...
    <p>You can't just softmax each tile independently&mdash;the denominators would be wrong. Instead, for each query position, the implementation tracks a running max and a running sum-of-exponentials across all key tiles. When a new tile has a larger max, it corrects the accumulated values with <code>exp(old_max - new_max)</code>. This produces numerically identical results to computing the full softmax.</p>
    <h3>When it falls back to standard attention</h3>
    <p>If the sequence length is less than or equal to <code>tile_size</code>, there's nothing to tile&mdash;the implementation just does regular attention. The tiling only kicks in for longer sequences.</p>
    <h3>Parameters</h3>
    <ul>
      <li><strong>embed_dim</strong>, <strong>num_heads</strong> &mdash; same as standard attention</li>
      <li><strong>tile_size</strong> &mdash; controls the tile dimensions. Smaller tiles use less memory but have more loop overhead. Default 32 is reasonable; try smaller values for very long sequences.</li>
    </ul>
  ...
  ADWeightTying: `
    ...
    <p>The embedding matrix <code>W</code> has shape <code>[vocab_size x embed_dim]</code>. To get logits from hidden states <code>[embed_dim x seq_len]</code>, we just compute <code>W @ hidden</code>, which gives <code>[vocab_size x seq_len]</code>&mdash;a logit score for each vocab token at each position.</p>
    ...
    <h3>Inputs</h3>
    <ul>
      <li><strong>input</strong> &mdash; hidden states, shape <code>[embed_dim x seq_len]</code></li>
      <li><strong>weights</strong> &mdash; the embedding weight matrix (connect to the <code>weights</code> output of an ADEmbedding node)</li>
    </ul>
    <h3>When to use</h3>
  ...
  ADRepetitionPenalty: `
    ...
    <p>Tokens not in the list are left unchanged. This asymmetric treatment (dividing positives, multiplying negatives) comes from the CTRL paper and handles the sign correctly&mdash;both operations reduce the token's probability after softmax.</p>
    <h3>Choosing the penalty value</h3>
    <ul>
      <li><code>1.0</code> &mdash; no effect</li>
      <li><code>1.1-1.3</code> &mdash; mild, discourages exact repetition without changing the output much</li>
      <li><code>1.5-2.0</code> &mdash; aggressive, noticeably changes the token distribution</li>
      <li><code>&gt;2.0</code> &mdash; very strong, can make the output incoherent by suppressing common tokens too hard</li>
    </ul>
    <h3>Inputs</h3>
    <ul>
      <li><strong>logits</strong> &mdash; model output logits <code>[vocab_size x seq_len]</code></li>
      <li><strong>generated_ids</strong> &mdash; list of token IDs the model has produced so far</li>
    </ul>
  `,
};
